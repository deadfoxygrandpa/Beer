...
type alias Rendertron =
    Automaton Model.State Element


type alias HiddenState a =
    { input : a, render : a -> Element, output : Element }
...
tail list =
    case List.tail list of
        ...
        Nothing ->
            Debug.crash "tail of empty list"
            ...
            in
                if a == s.input then
                    ...
                    let
                        o = renderer a
                    in
                        ( o, { s | input = a, output = o } )
    in
        hiddenState state' step


renderer : List Rendertron -> Automaton Model.State (List Element)
renderer rendertrons =
    Automaton.combine rendertrons


renderLines : Automaton Model.State (List Element) -> Signal Model.State -> Signal Element
renderLines renderer state =
    Signal.map (flow down) (run renderer [] state)
...
renderGame seed renderer state dimensions =
    let
        instructions = constant Interface.instructions
    ...
    in
        Signal.map layers (Signal.map3 (\a b c -> [ a, b, c ]) instructions distorted seed')
...
lines initialState =
    ...
    , rendertron
        ...
        initialState
    , rendertron
        (\state -> ( state.person.weight, state.person.orientation, state.person.gender, state.person.sex, state.person.alcoholism ))
        (\( weight, orientation, gender, sex, alcoholism ) ->
            flow
                right
                [ plainText "you are a "
                , plainText << String.left 5 << toString <| weight
                , plainText "kg "
    ...
    , rendertron
        ...
        initialState
    , rendertron
        (\state -> Interface.timeDisplay state.elapsed)
    ...
    , rendertron
        ...
        initialState
    , rendertron
        (\state -> ( state.person.conscious, state.person.alive ))
        (\( conscious, alive ) ->
            if not alive then
