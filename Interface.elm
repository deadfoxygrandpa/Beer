...
makeButtons initial dimensions names =
    let
        inputs = map (\_ -> Signal.mailbox initial) names
    ...
    in
        ( elems, Signal.mergeMany <| map .signal inputs )


keyPressed : Char -> Signal ()
keyPressed char =
    keyCodePressed <| Char.toCode char


keyCodePressed : Char.KeyCode -> Signal ()
keyCodePressed code =
    Discrete.whenChangeTo True <| Keyboard.isDown code


latch : Signal a -> Signal Bool
latch sig =
    Discrete.folde not False <| Discrete.whenChange sig
...
box s ( w, h ) c =
    color black
        ...
        <| (fromString >> leftAligned) s


( timeAccelerationButtons, timeAcceleration ) =
    makeButtons "1" ( 40, 30 ) [ "1", "2", "60", "600" ]
...
( chugButton, chugClicks ) =
    ( Input.button (Signal.message chug.address ()) "slam back a brewski", chug.signal )


( sipButton, sipClicks ) =
    ( Input.button (Signal.message sip.address ()) "sip your beer", sip.signal )


( gulpButton, gulpClicks ) =
    ( Input.button (Signal.message gulp.address ()) "gulp down some beer", gulp.signal )


( urinateButton, urinateClicks ) =
    ( Input.button (Signal.message urinate.address ()) "urinate", urinate.signal )


( orderButton, orderClicks ) =
    ( Input.button (Signal.message order.address ()) "another pint, please", order.signal )


( orderButton2, orderClicks2 ) =
    ( Input.button (Signal.message order2.address ()) "order from the menu", order2.signal )


timeFactor : Signal Time -> Signal Time
timeFactor time =
    Signal.map2 (\x t -> inHours (1000 * (withDefault 1 << Result.toMaybe <| String.toFloat x) / (1000 / t))) timeAcceleration time
...
instructions =
    ...
    in
        flow
            right
            [ container 100 (heightOf controls) middle <| (fromString >> leftAligned) "controls: "
            , controls
            ]
...
render state ( w, h ) seed =
    let
        messages = container w h (midTopAt (relative 0.5) (relative 0.7)) <| flow down <| map ((fromString >> leftAligned) << .msg) (take 5 state.messages)

        gameScreen =
            container w h middle
                <| flow
                    down
                    [ flow right [ chugButton, (fromString >> leftAligned) " time acceleration: ", flow right timeAccelerationButtons ]
                    ...
                    , (fromString >> leftAligned)
                        <| peeDisplay state.person.urine state.person.wetSelf
                        ++ (if state.person.urinating then
                                " (you are peeing)"
                            else
                                ""
                           )
    ...
    in
        layers [ instructions, distorted, container w h bottomLeft ((fromString >> leftAligned) <| "random seed: " ++ toString seed) ]


gameScreen : Model.State -> ( Int, Int ) -> Element
gameScreen state ( w, h ) =
    container w h middle
        <| flow
            down
            [ flow right [ chugButton, (fromString >> leftAligned) " time acceleration: ", flow right timeAccelerationButtons ]
