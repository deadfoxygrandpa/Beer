...
getLeft (Zipper a _ _) =
    a


getRight : Zipper x -> List x
getRight (Zipper _ _ b) =
    b
...
type alias Menu a =
    { title : String, items : Zipper a }


menu : Menu Model.Beer
menu =
    Menu "Beer Menu" <| fromList BeerList.allBeers
...
moveUp _ menu =
    { menu | items = withDefault menu.items <| left menu.items }


moveDown : a -> Menu b -> Menu b
moveDown _ menu =
    { menu | items = withDefault menu.items <| right menu.items }


render : Signal.Mailbox a -> (a -> String) -> Menu a -> ( Int, Int ) -> Element
render clicker toString { title, items } ( w, h ) =
    let
        choice item = button item

        button item =
            Input.customButton
                (Signal.message clicker.address item)
        ...
        choices =
            container w h middle
                <| flow down
                << map (container w 30 middle)
                <| map (choice) (reverse <| getLeft items)
