...
initialState =
    Model.State (fst <| Random.generate Randomize.person gen) 0 0 0 [ Model.Message "Bartender: welcome" 5 ]


time : Signal Time
time =
    Interface.timeFactor frames
...
updates =
    let
        step =
            mergeMany
                [ Signal.map Update.sip (keepWhen Keyboard.space 0 time)
    ...
    in
        Signal.map2 (,) step time


stateSignal : Signal Model.State
stateSignal =
    foldp Update.updateState initialState updates'


initialGameState : Model.GameState
initialGameState =
    Model.GameState False False


gameStateUpdates : Signal (Model.GameState -> Model.GameState)
gameStateUpdates =
    mergeMany
        [ Signal.map Update.togglePause (Interface.keyPressed 'P')
...
gameStateSignal =
    foldp Update.updateGameState initialGameState gameStateUpdates


gameGroup : InputGroup b
gameGroup =
    makeGroup (Signal.map (\g -> not << (\( a, b ) -> a || b) <| ( g.paused, g.menuOpen )) gameStateSignal)


updates' : Signal ( Model.State -> Model.State, Float )
updates' =
    gameGroup.add updates ( Update.emptyFrame 0, 0 )
...
menuGroup =
    makeGroup (Signal.map .menuOpen gameStateSignal)


menuUpdates' : Signal (Menu.Menu Model.Beer -> Menu.Menu Model.Beer)
menuUpdates' =
    menuGroup.add menuUpdates identity


menuUpdates : Signal (Menu.Menu Model.Beer -> Menu.Menu Model.Beer)
menuUpdates =
    mergeMany
        [ Signal.map Menu.moveUp <| Interface.keyCodePressed 38
...
menuSignal =
    foldp Menu.update initialMenu menuUpdates'


currentBeer : Signal Model.Beer
currentBeer =
    merge
        (Signal.map (Menu.select << .items) (sampleOn (menuGroup.add (Interface.keyCodePressed 13) ()) menuSignal))
        beerInput.signal


beerInput : Signal.Mailbox Model.Beer
beerInput =
    Signal.mailbox (snd initialState.person.beers)
...
showBeer beer =
    beer.name ++ " : a" ++ quality beer.score ++ toString beer.style ++ " from " ++ beer.brewery ++ " (" ++ toString beer.abv ++ "% ABV)"


menuScreen : Signal Element
menuScreen =
    Signal.map2 (Menu.render beerInput showBeer) menuSignal Window.dimensions


fpsBar : Signal Element
fpsBar =
    Signal.map2 (\w t -> color red <| spacer (clamp 0 w << round <| t / Constants.framerate * (toFloat w)) 2) Window.width feeps


main : Signal Element
main =
    (\g m x b dim person t ->
        let
            screen =
                if x then
                    m
                    ...
                    g
        in
            SpecialEffects.theBest (Model.Environment dim (person.bac / person.alcoholism) <| toFloat t) <| layers [ screen, b ]
    )
        <~ gameScreen
        ~ menuScreen
        ~ (.menuOpen <~ gameStateSignal)
        ~ fpsBar
        ~ Window.dimensions
        ~ (.person <~ stateSignal)
        ~ (count <| feeps)
...
gameScreen =
    Rendertron.renderGame seed (Rendertron.renderer <| Rendertron.lines initialState) stateSignal Window.dimensions


feeps : Signal Float
feeps =
    Automaton.run (Automaton.average 30) 0 ((\t -> 1000 / t) <~ (fps Constants.framerate))


count : Signal a -> Signal Int
count a =
    Automaton.run Automaton.count 0 a
