        ...
        bw =
            case person.sex of
                ...
                Model.Female ->
                    0.49
    in
        (10 * bac * bw * person.weight) / (1.2 * 0.806)
...
updateState ( step, timeStep ) state =
    let
        state' =
            if state.person.conscious then
                step state
            else
                state
...
consume rate timeStep state =
    ...
    in
        { state | person = person, drinks = state.drinks + (volume / 355) }


sip : Float -> Model.State -> Model.State
sip =
    consume Constants.sipRate


gulp : Float -> Model.State -> Model.State
gulp =
    consume Constants.gulpRate


chug : Float -> Model.State -> Model.State
chug =
    consume Constants.chugRate


urinate : a -> Model.State -> Model.State
urinate _ state =
    if (state.person.urine < 10) then
        state
    else
        let
            person = state.person
...
order beer state =
    if (fst state.person.beers) > 10 then
        addMessage "Bartender: finish that one first!" 5 state
    else
        let
            person = state.person
        ...
        in
            addMessage "Bartender: enjoy!" 5 { state | person = person' }


orderAnother : a -> Model.State -> Model.State
orderAnother _ state =
    order (snd state.person.beers) state


addMessage : String -> Time -> Model.State -> Model.State
addMessage s t state =
    { state | messages = Model.Message s t :: state.messages }
...
process timeStep person =
    let
        ...
        urinated =
            if (person.urinating) then
                Constants.urinationRate * timeStep
            else
                0
